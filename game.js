...
export class Game extends Phaser.Scene {
    ...
    init() {//se ejecuta solo una vez al inicio
        this.glueRecordVelocityX = INITIAL_VELOCITY_X;//guarda la velocidad de la bala para cuando se pega a la nave
    ...
    preload() {//precarga cosas
      let url = 'https://raw.githubusercontent.com/rexrainbow/phaser3-rex-notes/master/dist/rexvirtualjoystickplugin.min.js';//para usar el joistick
      this.load.plugin('rexvirtualjoystickplugin', url, true);//impplementa un joystick
      ...
      this.load.atlas('bulletTot','images/bullet/bulletTot.png','images/bullet/bulletTot.json');//sprite
    ...
    create() {
        ...
        this.joyStick = this.plugins.get('rexvirtualjoystickplugin').add(this, {
          ...
          forceMin: 50,//fuerza minima hacia un lado para que se mueva
    }).on('update', this.dumpJoyStickState, this);

    this.joystickCursors = this.joyStick.createCursorKeys();//contiene la informaciion de direccion del cursor. up, right, left y down
    ...
    update() 
    {
      this.naveP.updatePosition(this.bullet,this.cursors,this.joystickCursors);//mover
        ...
        if (this.cursors.up.isDown||this.joystickCursors.up.isDown ) {//tecla ariba
  ...
  endGame(completed)
  {
    ...
    dumpJoyStickState() {//comprobar los resultados lanzados por el joystick
      var cursorKeys = this.joyStick.createCursorKeys();
      console.log('Force: ', this.joyStick.force);//me da la fuerza aplicada
      console.log('Angle: ', this.joyStick.angle);//me da el aungulo
    }
}
